...
app.get('/guessMade',function(req,res){
  var result = connection.query("SELECT randomNumberGenerated FROM guesses WHERE gameId="+req.query.gameId);

  var numberToGuess = result[0].randomNumberGenerated;
  ...
  if (numberGuessed == numberToGuess) {
    connection.query("DELETE FROM guesses WHERE gameId="+req.query.gameId);
