...
app.get('/sayhello', function(req,res){
  var namePassed = req.query.namePassed;
  ...
  if (namePassed =="") {
    res.send('Hello stranger.  This is request number ' + helloRequestsReceived);	
  } 
  
  else if (isNaN(namePassed)) {
    res.send("Hello "+namePassed+". This is request number "+helloRequestsReceived);
  } 
  
  else {
    const result = connection.query("SELECT name FROM students WHERE Tuid=" + namePassed);
    //console.log(result);
    if (result.length == 0) {
      res.send("Hello person with an invalid TUid. This is request number "+helloRequestsReceived);
    } else {
      res.send("Hello "+result[0].name+". This is request number "+helloRequestsReceived);
    }
  }
});
